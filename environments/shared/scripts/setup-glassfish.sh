...
function installLocalGlassFishJavaEE {
	echo "===================="
...
function installRemoteGlassFishJavaEE {
	echo "====================="
	...
	installLocalGlassFishJavaEE 
}

function setupEnvVars {
	echo "creating glassfish environment variables"
...
function createModifiedAccFile {
	...
	# con 
	# - <target-server name="glassfish" address="10.11.1.111" port="3700"/>
	sed -r 's#(<target-server name)="sc11152545.us.oracle.com" address="localhost" (port="3700"/>)#\1="glassfish" address="10.11.1.111" port="3700"/>#' $INFILE > $OUTFILE
#	sed -r 's#<target-server name="sc11152545.us.oracle.com" address="localhost" port="3700"/>#<target-server name="glassfish" address="10.11.1.111" port="3700"/>#' $INFILE > $OUTFILE
}
...
function setupGlassFishJavaEE {
	...
	cp ${AS_HOME}/lib/appclient.jar ${VAGRANT_DOWNLOADS}/appclient-10.11.1.111.jar 
}

function installGlassFishJavaEE {
	if downloadExists $JAVAEE_ARCHIVE; then
		installLocalGlassFishJavaEE
	else
		installRemoteGlassFishJavaEE
	fi
}
...
installGlassFishJavaEE
setupEnvVars
setupGlassFishJavaEE
